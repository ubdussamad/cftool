let params = new URLSearchParams(location.search);
  ...
  d3.json(params.get('tree'), function(treeData) {
    ...
    // This only works with https connections.
    function dbclk_cpy(this_) {
        navigator.clipboard.writeText(this_.title)
        .then(() => {
    ...
    function update_details_panel (lineage_str) {
      ...
      d3.keys(node_data.key_regs).forEach(function(key_reg) {
        ...
        cell1.addEventListener("dblclick", function() { dbclk_cpy(this); } ); 
    ...
    function render_graphs () {
      ...
      document.getElementById('key_reg_property_selection_ddm').onchange = function() {
        ...
        for (var i = 0; i < len_list; i++) {
          ...
          cell1.title = prop_list[i] + "| Double click to copy.";
          
          cell1.onclick = function() {
            navigator.clipboard.writeText(this.title.split("|")[0])
            .then(() => {
              console.log('Async: Copying to clipboard was successful!');
            })
            .catch(() => {
